...
- Conclude with forward-looking insights and discussion points"`;
        ...
        const formattedContext = formatConversationContext(request.conversationContext);
        const hasContext = formattedContext !== "No previous conversation context available.";
...
This is a fresh conversation with no previous context. Transform this into a comprehensive, detailed prompt that will get excellent results. Remember to make reasonable assumptions rather than asking questions, and create a complete, ready-to-use prompt.`;
                ...
                try {
                    ...
                    console.log(`Background: Enhancement completed in ${elapsedTime.toFixed(2)} seconds`);
                    ...
                    if (fetchError.name === 'AbortError') {
                        ...
                        sendResponse({ error: "Network error: Check your internet connection" });
                    } else {
                        sendResponse({ error: `API request failed: ${fetchError.message}` });
                    }
                }
            } catch (error) {
