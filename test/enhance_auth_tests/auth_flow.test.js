...
const SUPABASE_SERVICE_ROLE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImV4cCI6MTk4MzgxMjk5Nn0.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU"; // Default local service role
...
async function checkSignupEvent(userId) {
  ...
  if (events && events.length > 0) {
    console.log(
      `✅ Found signup event with ${events[0].credits_granted} credits granted`,
    );
    return events[0];
  }
...
async function testEnhance(
  ...
  try {
    ...
    console.log(
      `   Status: ${response.status}, Error Code: ${response.data.code || "N/A"}`,
    );
  } catch (err) {
...
async function runTests() {
  ...
  console.log(
    "ℹ️  Testing V2A-01 (Auth enforcement) and V2A-03 (Credit granting)\n",
  );

  try {
    // Test 1: No Auth Header
    await testEnhance("No Authorization header", null, 401, "AUTH_REQUIRED");
    ...
    if (signupEvent) {
      assert.strictEqual(
        signupEvent.credits_granted,
        25,
        `Expected 25 credits granted in event, got ${signupEvent.credits_granted}`,
      );
    ...
    console.log(
      "   1. Use your extension UI to set up a user with a properly encrypted API key",
    );
