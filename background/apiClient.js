...
export async function callOpenAI(apiKey, systemInstruction, userPrompt) {
    ...
    try {
        ...
        if (DEBUG) console.log(`[apiClient] API response status: ${response.status}`);
        ...
        if (!response.ok) {
            ...
            try {
                ...
                try {
                   ...
                   console.error(`[apiClient] API error (${response.status}), could not parse JSON body:`, errorText);
                   errorBody = { error: { message: errorText || "Failed to read error body." } }; // Mimic OpenAI error structure
                } catch (textError) {
                   console.error(`[apiClient] API error (${response.status}), failed to read error body as text:`, textError);
            ...
            if (errorBody?.error?.message) {
                ...
                errorMessage += ". Please check API key, model access, or OpenAI status.";
            }
            // Add specific guidance for common errors
            if (response.status === 401) errorMessage += " (Invalid API Key?)";
            if (response.status === 429) errorMessage += " (Rate limit exceeded or quota issue?)";
            if (response.status === 404) errorMessage += " (Model not found or API endpoint issue?)";
        ...
        if (data.choices && data.choices.length > 0 && data.choices[0].message) {
            if (DEBUG) console.log("[apiClient] Received response: ", data.choices[0].message.content);
            return data.choices[0].message.content;
        } else {
            console.error("[apiClient] Invalid response structure from OpenAI:", data);
        ...
        if (error.name === 'AbortError') {
            console.error("[apiClient] OpenAI API request timed out after 50 seconds.");
