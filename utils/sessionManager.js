...
export class SessionManager {
  constructor() {
    ...
    // Logging
    this.debug = false; // Can be configured
  }
  ...
  _validateSession(session) {
    ...
    if (!session || typeof session !== "object") {
      return {
        isValid: false,
        needsRefresh: false,
        reason: "Session is null or not an object",
      };
    }
    ...
    if (expiresAt <= now + this.expiryBuffer) {
      return {
        isValid: false,
        needsRefresh: true,
        reason: "Session expires within buffer time",
      };
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (
          ...
          this._log(
            "_doRefresh",
            "Authentication error, clearing session and not retrying",
          );
          await this._clearSession();
          return {
            success: false,
  ...
  _handleRefreshFailure() {
    ...
    this._log(
      "_handleRefreshFailure",
      `Consecutive failures: ${this.consecutiveFailures}/${this.maxFailures}`,
    );
  ...
  _isCircuitBreakerOpen() {
    ...
    if (Date.now() >= this.circuitBreakerResetTime) {
      this._log(
        "_isCircuitBreakerOpen",
        "Circuit breaker reset time reached, closing circuit breaker",
      );
      this.circuitBreakerResetTime = null;
      this.consecutiveFailures = 0;
      return false;
  ...
  async _getStoredSession() {
    try {
      ...
      this._log(
        "_getStoredSession",
        "Error retrieving session from storage",
        error,
      );
      return null;
    }
