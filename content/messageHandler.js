    ...
    case "CoPromptEnhanceRequest": // Message from injected.js asking us to enhance
      ...
      try {
        ...
        port.onDisconnect.addListener(() => {
          ...
          logMessageHandlerDebug(`[MessageHandler] Port disconnected. Error: ${lastErrorMessage || '(none)'}`);
          if (port) { // Check if we still expected the port to be valid
        ...
        port.onMessage.addListener((response) => {
          logMessageHandlerDebug(`[MessageHandler] Received message from background port (ID: ${requestId}):`, response);
          if (response.requestId === requestId) {
            // Forward the success or error response back to the injected script
            window.postMessage(response, "*");
            if (port) {
              logMessageHandlerDebug(`[MessageHandler] Disconnecting port cleanly (ID: ${requestId}).`);
              try { port.disconnect(); } catch (e) { console.warn("[MessageHandler] Error during explicit port disconnect:", e); }
              port = null; // Mark port as disconnected
            }
          } else {
            console.warn(`[MessageHandler] Received response for mismatched requestId. Expected: ${requestId}, Got: ${response.requestId}`);
          }
        });

        // Send the actual request TO the background script via the port
        logMessageHandlerDebug(`[MessageHandler] Posting ENHANCE_PROMPT message via port (ID: ${requestId}).`);
        port.postMessage({
          type: "ENHANCE_PROMPT",
