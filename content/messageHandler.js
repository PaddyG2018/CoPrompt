...
async function _handleEnhanceRequest(event) {
  ...
  try {
    const contextExtractorModule = await import(
      chrome.runtime.getURL("content/contextExtractor.js")
    );
    conversationContext = contextExtractorModule.getConversationContext();
    logDebug(
      "Captured Context (from module):",
  ...
  let timeoutId = setTimeout(() => {
    console.error(
      "MessageHandler: Background script response timed out after 55 seconds",
    );
    window.postMessage(
      {
        type: "CoPromptEnhanceResponse",
        error: "Background script response timed out (55s).",
      },
      "*",
    );
  ...
  try {
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        if (!timeoutId) {
          ...
          logDebug(
            "Timeout already occurred, ignoring response from background.",
          );
          return;
        }
        ...
        if (chrome.runtime.lastError) {
          console.error(
            "MessageHandler: chrome.runtime.sendMessage callback error:",
            chrome.runtime.lastError.message,
          );
          let errorMessage =
            "An error occurred communicating with the background service.";
          if (
          ...
          window.postMessage(
            { type: "CoPromptEnhanceResponse", error: errorMessage },
            "*",
          );
          return;
        }
        ...
        logDebug(
          `Received response from background script after ${requestTime.toFixed(2)} seconds:`,
          response,
        );

        if (!response) {
          console.error(
            "MessageHandler: No response received from background script",
          );
          window.postMessage(
            {
              type: "CoPromptEnhanceResponse",
        ...
        if (response.error) {
          ...
          console.error(
            "MessageHandler: Invalid response format from background script",
            response,
          );
          window.postMessage(
            {
              type: "CoPromptEnhanceResponse",
              error: "Invalid response format from background.",
            },
            "*",
          );
        }
    ...
    console.error(
      "MessageHandler: chrome.runtime.sendMessage synchronous error:",
      error,
    );
    if (timeoutId) {
      clearTimeout(timeoutId);
    ...
    if (error.message?.includes("Extension context invalidated")) {
      errorMessage =
        "Connection to background service failed. Please try again.";
    }
    // Send error back to injected script
    window.postMessage(
      { type: "CoPromptEnhanceResponse", error: errorMessage },
...
async function _handleDomUpdateResponse(event) {
  logDebug(
    "Handling DOM update response originating from injected script:",
    event.data,
  );
  ...
  const { findActiveInputElement, updateInputElement } = await import(
