...
async function determinePromptCategory(prompt) {
    ...
    const codePatterns = /\bcode\b|\bfunction\b|\bclass\b|\bprogramming\b|\bjavascript\b|\bpython\b|\bc\+\+\b|\bjava\b|\bhtml\b|\bcss\b|\bsql\b/i;
    const researchPatterns = /\bresearch\b|\bstudy\b|\banalysis\b|\bdata\b|\bscientific\b|\bhistory\b|\bexplain\b|\btheory\b/i;
    const creativePatterns = /\bstory\b|\bcreative\b|\bwrite\b|\bpoem\b|\bnarrative\b|\bfiction\b|\bimagine\b|\bscenario\b/i;
...
async function getAPIKeyFromContentScript() {
    return new Promise((resolve) => {
        ...
        const handleMessage = (event) => {
            if (event.source !== window || event.data.type !== "CoPromptAPIKeyResponse") return;
...
- End with a resolution that establishes a new understanding between species"`;
        ...
        return new Promise((resolve) => {
            ...
            const handleEnhanceResponse = (event) => {
                if (event.source !== window || event.data.type !== "CoPromptEnhanceResponse") return;
                
                // Calculate response time
                const responseTime = (Date.now() - requestStartTime) / 1000;
                console.log(`Received enhancement response after ${responseTime.toFixed(2)} seconds`);
                ...
                if (event.data.error) {
                    ...
                    if (enhancedPrompt === prompt || enhancedPrompt.length < prompt.length) {
                        console.warn("Enhanced prompt was not meaningful, returning original");
                        resolve(prompt);
                    } else {
            ...
            const checkInterval = setInterval(() => {
                ...
                console.log(`Still waiting for enhancement response after ${waitTime.toFixed(0)} seconds...`);
                
                // After 30 seconds, warn the user that it's taking longer than expected
                if (waitTime > 30 && waitTime < 31) {
                    console.warn("Enhancement request is taking longer than expected. This might be due to high API traffic or a slow connection.");
                }
