...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const {
      data: { user },
      error: authError,
    } = await supabase.auth.getUser(); // Pass no arg, client uses its configured auth
    ...
    if (req.method === "GET") {
      ...
      if (dbError) {
        console.error(
          `Error checking API key status for user ${userId}:`,
          dbError,
        );
        // Return false by default on error, or you could return a 500
        return new Response(
          JSON.stringify({ hasKey: false, error: "Database query failed." }),
          {
      ...
      return new Response(JSON.stringify({ hasKey: !!data }), {
        // !!data will be true if data is not null/undefined
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (req.method === "POST") {
      ...
      if (plaintextApiKey === null || plaintextApiKey === "") {
        // Check for null or empty string for clearing
        ...
        if (deleteError) {
          console.error(
            `Error deleting API key for user ${userId}:`,
            deleteError,
          );
          return new Response(
      ...
      if (
        typeof plaintextApiKey !== "string" ||
        !plaintextApiKey.startsWith("sk-")
      ) {
        return new Response(
          JSON.stringify({ error: "Invalid API key format provided." }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
      ...
      if (upsertError) {
        console.error(
          `Error upserting API key for user ${userId}:`,
          upsertError,
        );
        return new Response(
    ...
    console.error(
      "Unexpected error in store-user-api-key function:",
      error,
      error.stack,
    );
    return new Response(
